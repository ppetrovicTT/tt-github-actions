    ...
    for match in re.finditer(r"non-default args:\s*\{", log_content):
        ...
        if args_str:
            try:
                ...
                json_str = args_str.replace("'", '"').replace("True", "true").replace("False", "false").replace("None", "null")
    ...
    for match in re.finditer(cli_pattern, log_content, re.IGNORECASE):
        ...
        if any(kw in param.lower() for kw in _TT_CLI_KEYWORDS):
            if param not in examples:
                examples[param] = []
            examples[param].append(ConfigExample(
                param_name=param,
                value=value,
                source="CLI argument",
        ...
        if isinstance(value, dict):
            ...
            for nested_key, nested_value in value.items():
                ...
                if not any(e.value == str(nested_value) for e in examples[nested_key]):
                    examples[nested_key].append(ConfigExample(
            ...
            if not any(e.value == str(value) for e in examples[key]):
                examples[key].append(ConfigExample(
                    param_name=key,
                    value=str(value),
                    source=source,
                    context=context,
    ...
    for repo_name, repo_path in repo_paths.items():
        ...
        try:
            result = subprocess.run(
                ["grep", "-r", "-l", "--include=*.yaml", "--include=*.yml",
                 "--include=*.json", "--include=*.py", "--include=*.md",
                 param_name, repo_path],
                capture_output=True,
                text=True,
                timeout=10,
            )
...
def _get_file_context(file_path: str, param_name: str, context_lines: int = 3) -> str | None:
    ...
    try:
        result = subprocess.run(
            ["grep", "-n", "-B", str(context_lines), "-A", str(context_lines),
