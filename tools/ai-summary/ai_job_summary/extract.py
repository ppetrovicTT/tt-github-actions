...
def get_job_status(extracted_log: ExtractedLog) -> JobStatus:
    ...
    if extracted_log.exit_code and extracted_log.exit_code != 0:
...
def extract_log(
    ...
    result.deduplicated_warnings = {
        w: c for w, c in warning_counts.most_common(10) if c > 1
    }
...
def format_extracted_log(extracted: ExtractedLog) -> str:
    ...
    elif extracted.exit_code and extracted.exit_code != 0:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        extracted = extract_log(log_path)
        print(format_extracted_log(extracted))
