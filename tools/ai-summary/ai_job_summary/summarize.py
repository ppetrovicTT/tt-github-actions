    ...
    if extracted_log.config_attributions:
        ...
        for attr in extracted_log.config_attributions[:3]:  # Limit to top 3
            if attr.source_config:
                ...
                hint_lines.append(
                    f"  - `{attr.error_param_name}` set by {layer_name} layer "
                    f"(value: {attr.source_config.value})"
                )
                if attr.explanation:
...
def format_summary_markdown(
    ...
    if extracted_log and extracted_log.job_url:
        job_label = extracted_log.job_name or "View Job"
        md = f"### {_emoji(job_status.status_code)} {job_status.status_text} ([{job_label}]({extracted_log.job_url}))\n"
    else:
