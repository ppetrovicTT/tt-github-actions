...
def _job(status="SUCCESS", category="", job_name="job", job_url="", root_cause="",
         source_stem="65000000001", job_id="", is_your_code=None):
...
class TestExtractModel:
    ...
    def test_extracts_from_root_cause_when_job_name_generic(self):
        job = _job(job_name="run-tests-with-inference-server",
                   root_cause="Tokenizer failed for Llama-3.3-70B-Instruct model")
...
class TestFormatRunReport:
    ...
    def test_failed_job_link_uses_job_id_and_proper_url(self):
        job = _job("CRASHED", job_name="run-tests-with-inference-server",
    ...
    def test_run_column_shows_model_and_device(self):
        job = _job("CRASHED", job_name="run-release-Llama-3.1-8B-Instruct-n150-n150",
                   source_stem="65000099")
        stats = _stats(jobs=[job])
        report = format_run_report(stats)
